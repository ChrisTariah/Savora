...
## Database
The database is an SQLite local disk database created from scratch using the script [setup_database.py](server/static/setup_database.py) and the file [Base_Schema.sql](server/static/Base_Schema.sql). Within [setup_database.py](server/static/setup_database.py) is the list of files that will be run to build the database. This list must always include [Base_Schema.sql](server/static/Base_Schema.sql) and may include additional files to modify or populate the database such as [Test_Data.sql](server/static/Test_Data.sql):
``` python
SQL_FILES = [
...
Within [Base_Schema.sql](server/static/Base_Schema.sql) there are some lines commented out as they may not be implemented until an ingredient API is added to populate the table with the relevant information. Permanent changes to the database should be added to the [Base_Schema.sql](server/static/Base_Schema.sql) file, rather than in a new separate file, but temporary changes for testing should remain in separate files and added to the SQL_FILE list in [setup_database.py](server/static/setup_database.py).

#### Query Library
The file [Query_Library.sql](server/static/Query_Library.sql) that has some queries that may be useful to the site, such as filtering by price or showing only recipes the user has ingredients for. More queries can be added by request. Note that each query will have a line with a comment stating where a variable should be used like:
``` sql
...
The file [database_terminal_session.py](server/database_terminal_session.py) which will allow you to open the [Savora.db](server/Savora.db) file and interact with it using SQLite commands for testing and development purposes.
